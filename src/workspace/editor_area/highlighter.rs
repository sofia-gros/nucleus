...
fn semantic_name(scope: Scope) -> Option<&'static str> {
        ...
        || scope.starts_with("support.type")
    {
        ...
        "constant"
    } else if scope.starts_with("support.type.property-name") || scope.starts_with("entity.name.tag") || scope.starts_with("entity.other.attribute-name") {
        "property"
...
#[derive(Default)]
pub struct ShowcaseHighlightStyles;
...
impl HighlightStyleResolver for ShowcaseHighlightStyles {
    fn style(&self, name: &str) -> Option<HighlightStyle> {
        let color = match name.split('.').next()? {
            ...
            "type" => 0x6f42c1,
            "variable" => 0x333333,
            "property" | "entity" => 0x8b8000,
            "punctuation" => 0x555555,
            _ => return None,
        };
        Some(HighlightStyle {
